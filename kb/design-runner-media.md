...
| Claude Code | `2.1.191` | `--model`, `--fallback-model`, named/resumable sessions, `--output-format json`, `--max-turns` | **`--print --output-format json` result (fire-verified 2026-06-28):** carries `total_cost_usd` (spend), token `usage`, `modelUsage[model].contextWindow` (context). The head-less spend+context source. (`statusLine` carries `rate_limits`/`cost`/`context` JSON too, but it is a TUI footer that **does NOT fire under `claude --print`** — verified by probe — so it never reaches brr's daemon; `statusline.py` is dead head-less.) Also: `--max-budget-usd`, API rate-limit headers, Admin API reports. | Subscription 5h/weekly quota is **not** head-less-readable (statusLine TUI-only). Adopting result-JSON changes the stdout contract (stdout→JSON; parse `.result` for the reply) — deferred. |
...
| Readable local gauge (Claude) | **`claude --print --output-format json` result** — `total_cost_usd` (spend), `usage`, `modelUsage.contextWindow` | spend + context only; NO subscription quota (statusLine carries it but is TUI-only, never fires under `--print`). Deferred: adopting it changes the stdout contract. |
