...
- Gates are transport adapters; `telegram`, `slack`, and `github` ship
  ...
  card via `render_update`; the GitHub gate polls label-on-issue,
  mention-in-comment, or broad `any` activity, posts replies as
  PR/issue comments, and passes through `branch_target` for PR and
  PR-comment events so the sync hook refreshes the PR head before the
  worker runs.
...
- The GitHub gate polls the REST API (stdlib `urllib` only) for
  three trigger surfaces: `label-on-issue`, `mention-in-comment`,
  and broad `any` activity. It posts replies as comments on the
  originating issue or PR. PR and PR-comment events carry the PR
  head branch as `branch_target` so the daemon's pre-task fetch+ff
  refreshes that branch before the worker runs. Auth resolution at
  setup time: `gh auth token`, then `GITHUB_TOKEN` / `GH_TOKEN`,
  then interactive paste. State lives at `.brr/gates/github.json`.
  No webhooks in v1.
...
- If a file talks about transport, auth, polling, or delivery, jump to
  [gates](../src/brr/gates/). For label-on-issue,
  mention-in-comment, broad `any` activity, or PR / PR-comment events
  carrying `branch_target`, [gates/github.py](../src/brr/gates/github.py)
  specifically.
