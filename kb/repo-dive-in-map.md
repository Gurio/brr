...
- Live run UX is remote-first: gates render a per-task progress card from `UpdatePacket`s via the `run_progress` projection. There is no local status/inspect command; task context bundles and `.brr/` runtime artifacts are the forensics surface.
...
- Older public diagnostic commands (`status`, `inspect`, `docs`, `streams`, `stream`, `eject`) are intentionally absent; CLI tests assert they are not registered.
...
- `RunProgressView` (in `run_progress.py`) folds conversation records into a compact per-task projection that gates render today and future operator views can reuse. Adding new lifecycle UX should extend this projection, not reinvent rendering per gate.
...
- There is no hidden local status module. Humans debug failed runs from task manifests, conversation logs, responses, traces, worktrees, and preserved containers under `.brr/`; agents use the Task Context Bundle and generated run context file.
...
- Future operator views can reuse the verbose `render_text(..., compact=False)`
  form without inventing a second projection.
...
UX (gates and future operator views) should always go through
`run_progress`; introducing parallel ad-hoc derivations across modules
is the path to drift.
...
### No local status module

The remote gate is the primary surface for run progress. The old
`status.py` private helper is gone alongside the already-removed
`brr status` / `brr inspect` commands. Agents should orient from the
Task Context Bundle and generated run context file. Humans debugging a
failed run can inspect `.brr/tasks/`, `.brr/conversations/`,
`.brr/responses/`, and preserved traces/worktrees/containers directly.
