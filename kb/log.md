...
Opened a new kb area — Reviews — with one design artifact,
[`design-diffense.md`](design-diffense.md), proposed (not yet
accepted). The problem: generic PR tools are hostile to kb-first
reviews. ~Half of a brr PR's value lives in `kb/` diffs that read
poorly as raw unified diffs but well as rendered Markdown; tests
already encode user stories but read mechanically; and the reviewer
needs three things the diff view never gives — the surrounding mental
model, an honest picture of what becomes *possible*, and an honest
picture of what the *agent itself was uncertain about*.

What was learned / converged across four refinement passes: an
inspect-mode model of typed **item cards** with an always-vs-conditional
axes split and two-axis lore (descriptive + possibility, Souls-menu
framing); **walkthrough cards** as a first-class kind for cross-cutting
setup→action→outcome stories; **uncertainty cards** as a first-class
kind (assumption / concern / dilemma / out-of-scope-flag) read *first*,
so failure-mode honesty is structural rather than optional; usage demos
grounded in real test values; and a six-clamp discipline (sharp /
helpful / honest / non-prescriptive / emit-iff-honest /
substrate-honest) with an Occam's-razor reading-order test.

Cornerstone decided (as proposed): a two-layer architecture — a
language-agnostic JSON **pack** as the data layer, generated by the
runner at publish time, and a Python **substrate** as the renderer,
with multiple targets over the one pack (TUI via `brr review`, local
web via `textual serve`, hosted web via the brnrd dashboard's HTMX,
the forge PR body, and a future live agent). **Textual** is *proposed*
as the substrate for parallel CLI/web rendering, pending a validation
spike. The **PR body projection** is the committed v0 surface
(Uncertainty-at-top / Intent / Narrative / Touched / Reading order /
Deferred-or-open). The module/product is named **diffense** (pop-culture
alternatives `pensieve` / `holocron` dismissed as cosplay-leaning).
Likely lives in brr (`src/brr/diffense/`, `brr review`), wiring into the
publish kernel and the runner prompt.

What stays open (marked on the page): the pack JSON schema (finalised
after a hand-authored prototype against a real PR), substrate validation
(the Textual spike), aesthetic locking (validated alongside the spike),
and the project boundary (in-tree vs an extras-installed `brr-diffense`
package — leans in-tree). Explicitly *not* in this commit: no `brr
review` verb, no `src/brr/diffense/` module, no Textual code, no pack
schema, no runner-prompt or publish-kernel edits, no acceptance. Index
gets a new `## Reviews` section; this is the framing commit that opens
the exploration. diffense is the human-side counterpart to the
just-shipped gate-side `pr-review` event handling (brr reacts to
reviews; diffense produces surfaces humans can review well).
